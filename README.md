...
| 2 | **WooCommerce** | Standalone WordPress Plugin | Posts to the TackQuote API — not the WooCommerce REST API, which it never calls. The plugin (slug `tackquote-for-woocommerce`, v1.3.1) hooks `woocommerce_after_add_to_cart_button` for the buttons and `wp_footer` for the quote drawer, and hands order sync to **Action Scheduler** so a slow TackQuote can never sit inside a shopper's checkout. Verified end to end on WordPress 7.1 + WooCommerce 11.0.1; passes `wp plugin check` and `phpcs --standard=WordPress` clean. | [Download WooCommerce Plugin](https://github.com/ackm04/tack-ecommerce-extensions/releases/download/v1.1.0/tack-woocommerce.zip) (v1.3.1). For a **WordPress.org submission** use **[`tackquote-for-woocommerce.zip`](https://github.com/ackm04/tack-ecommerce-extensions/releases/download/v1.1.0/tackquote-for-woocommerce.zip)** instead — WordPress.org rejects the source archive above, which carries `bin/build.sh`. |
