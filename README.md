...
| 2 | **WooCommerce** | Standalone WordPress Plugin | Posts to the TackQuote API — not the WooCommerce REST API, which it never calls. The plugin (slug `tackquote`, v1.3.3) hooks `woocommerce_after_add_to_cart_button` for the buttons and `wp_footer` for the quote drawer, and hands order sync to **Action Scheduler** so a slow TackQuote can never sit inside a shopper's checkout. Verified end to end on WordPress 7.1 + WooCommerce 11.0.1; passes `wp plugin check` and `phpcs --standard=WordPress` clean. | **[`tackquote.zip` on the releases page](https://github.com/ackm04/tack-ecommerce-extensions/releases)** — built by `wordpress/tackquote/bin/build.sh`, which excludes `bin/`, `tests/` and `*.md`. This is the artifact to submit to WordPress.org and the one to install; the `tack-*-source.zip` archives elsewhere in this table are source, not installers. Link is deliberately unpinned: a version-pinned asset URL keeps returning 200 while serving an older build. |
| 3 | **Magento 2** | Native Magento 2 Extension | Connects via Magento REST Bearer API. The extension (`Tack_Quote`) registers custom XML layout blocks (`catalog_product_view.xml`) and posts quote requests to `/v1/widget/quotes`. | [Download Magento 2 Extension](https://github.com/ackm04/tack-ecommerce-extensions/releases/download/v1.1.0/tack-magento2.zip) |
