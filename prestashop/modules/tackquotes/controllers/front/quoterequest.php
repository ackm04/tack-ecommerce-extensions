...
class TackQuotesQuoteRequestModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
        ...
        if (Tools::getValue('ajax') === false && !Tools::isSubmit('email')) {
            $this->jsonError($this->module->l('Invalid request.'), 400);
            return;
        }
        ...
        if (!Validate::isEmail($email)) {
            $this->jsonError($this->module->l('A valid email address is required.'), 400);
            return;
        }

        $lineItems = $this->buildLineItems($productId, $quantity);
        if (empty($lineItems)) {
            $this->jsonError($this->module->l('No product to quote.'), 400);
            return;
        }
        ...
        if (!$apiKey) {
            $this->jsonError($this->module->l('TackQuote is not configured on this store yet.'), 503);
            return;
        }
